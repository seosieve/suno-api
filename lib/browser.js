...
async function uploadAudio(page, filePath) {
  ...
  for (let t = 0; t < 60; t++) {
    ...
    if (t % 5 === 0) console.log(`⏳ 업로드 중... ${t * 3}초`);
...
async function deleteSong(page, songName) {
  // .clip-row에서 Upload 뱃지가 있는 행을 우선 찾기
  let targetRow = page.locator('.clip-row:has-text("Upload")').filter({ hasText: songName }).first();

  if (!(await targetRow.isVisible({ timeout: 5000 }).catch(() => false))) {
    // Upload 뱃지 없으면 이름만으로 검색 (단일 삭제용)
    targetRow = page.locator(`.clip-row`).filter({ hasText: songName }).first();
  }

  if (!(await targetRow.isVisible({ timeout: 5000 }).catch(() => false))) {
    console.log(`⚠️ "${songName}"을 찾을 수 없습니다`);
    await page.screenshot({ path: "/tmp/suno_delete_notfound.png" });
    return false;
  }
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      const payload = {
        ...
        lang: process.env.BROWSER_LOCALE || "ko",
      };
      if (isDrag) {
...
async function handleCaptcha(page, createBtn) {
  ...
  for (let round = 0; round < 10; round++) {
    try {
      ...
      // iframe 자체가 사라졌는지 확인 (캡차 풀렸으면 종료)
      const iframeGone = await page.locator('iframe[title*="hCaptcha"], iframe[src*="hcaptcha"]').isVisible({ timeout: 2000 }).catch(() => false);
      if (!iframeGone) {
        console.log("  🎉 캡차 해결 완료!");
        return;
      }

      // challenge-container가 아직 보이는지 확인
      try {
        await challenge.waitFor({ state: "visible", timeout: 2000 });
      } catch (_) {
        console.log("  🎉 캡차 해결 완료!");
        return;
      }
      ...
      // 캡차가 사라졌는지 확인
      await page.waitForTimeout(3000);
      const stillVisible = await page.locator('iframe[title*="hCaptcha"], iframe[src*="hcaptcha"]').isVisible({ timeout: 2000 }).catch(() => false);
      if (!stillVisible) {
        console.log("  🎉 캡차 해결 완료!");
        return;
      }
